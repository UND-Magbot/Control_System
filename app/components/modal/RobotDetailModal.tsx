...
export default function RobotDetailModal({
    ...
    // open/selectedRobot 변경 시 폼 초기화 + edit mode 해제
    useEffect(() => {
    if (!isOpen) return;

    const rb =
        (selectedRobot as any)?.returnBattery ??
        (selectedRobot as any)?.return ??
        DEFAULT_RETURN_BATTERY;

    setDraft({
        operator: selectedRobot?.operator ?? "",
        serialNumber: selectedRobot?.serialNumber ?? "",
        model: selectedRobot?.model ?? "",
        group: selectedRobot?.group ?? "",
        softwareVersion: selectedRobot?.softwareVersion ?? "",
        site: selectedRobot?.site ?? "",
        registrationDateTime: selectedRobot?.registrationDateTime ?? "",
        returnBattery: typeof rb === "number" ? rb : DEFAULT_RETURN_BATTERY,
    });

    // 배터리 UI도 선택 로봇 값 기준으로 세팅
    setReturnBattery(typeof rb === "number" ? rb : DEFAULT_RETURN_BATTERY);
    setReturnBatteryText(String(typeof rb === "number" ? rb : DEFAULT_RETURN_BATTERY));
    lastValidRef.current = typeof rb === "number" ? rb : DEFAULT_RETURN_BATTERY;

    setIsEditMode(false);
    }, [isOpen, selectedRobotId, selectedRobot]);
    ...
    const handleSave = () => {
        // 배터리 입력값 확정(유효성 체크/되돌리기)
        const ok = validateAndFix();
        if (!ok) return;

        const rb = lastValidRef.current;
        const payload = { ...draft, returnBattery: rb };

        // TODO: 여기서 API 호출 (성공 시에만 보기모드 전환 권장)
        // await api.updateRobot(payload);

        // 저장 성공했다고 가정: draft에도 배터리 확정 반영
